...
def estimate_shipping_refund(days_in_transit: int, declared_delivery_days: int) -> dict:
    ...
    if overdue < 5:
        return {"eligible": False, "overdue_days": max(overdue, 0),
                "rule": "Повернення можливе лише при простроченні від 5 днів."}
    return {"eligible": True, "overdue_days": overdue,
            "rule": "Прострочення від 5 днів — клієнт має право на повернення вартості доставки."}
...
IMPL = {
    ...
    "estimate_shipping_refund": estimate_shipping_refund,
...
BASIC = ["get_order_status", "estimate_shipping_refund"]
...
# 1–2: подивитись і порахувати право на повернення, без дій (estimate_shipping_refund
# бере days_in_transit/declared_delivery_days з відповіді get_order_status — ланцюжок)
# 3: те саме право, але вже одним інструментом (фреймворк без дій)
